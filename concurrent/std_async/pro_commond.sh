...
# 定义要监控的进程名，可手动修改此变量来指定不同的进程
PROCESS_NAME="thread_pool_async_withwait"
...
function run_program() {
    ...
    CPUPROFILE="$GPERFTOOLS_DIR/prof.out" ./$PROCESS_NAME
}
...
function generate_gperftools_report() {
    ...
    pprof --pdf ./$PROCESS_NAME "$GPERFTOOLS_DIR/prof.out" > "$GPERFTOOLS_DIR/prof.pdf"
}
